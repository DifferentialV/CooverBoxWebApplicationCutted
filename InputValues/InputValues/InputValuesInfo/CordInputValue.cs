...
{
    public class CordInputValue : BaseInputValue
    {
        public string AddOption { get; set; }
        public bool AlwaysDefault { get; set; }
        public CordInputValue(System.Reflection.PropertyInfo valueInfo, string displayName) : base(valueInfo, displayName)
        {
            if (new[] { typeof(Cord) }.Contains(valueInfo?.PropertyType) == false)
            {
                throw new Exception($"{displayName} valueInfo должен указывать на поля типа Cord");
            }
        }
        ...
        public override void Bind(ModelBindingContext bindingContext, DBAppContext dbContext, MaterialsService materials)
        {
            ...
            Cord result = materials.Cords.FirstOrDefault(m => m.Id == resultId.Value);
            if (result == null)
            {
                if (Required == true && string.IsNullOrEmpty(AddOption))
                    bindingContext.ModelState.AddModelError(string.Empty, $"Поле {DisplayName} не удалось считать.");
