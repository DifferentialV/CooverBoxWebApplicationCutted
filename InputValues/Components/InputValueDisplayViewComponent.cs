...
{
    public class InputValueDisplayViewComponent : ViewComponent
    {
        public IViewComponentResult Invoke(dynamic model, InputValues.Base.BaseInputValue inputValue = null)
        {
            if (inputValue == null)
            {
                try
                {
                    ...
                    return View($"~/InputValues/Base/BaseDisplay.cshtml", inputValues);
                }
                catch(Exception e)
                {
            ...
            else
            {
                ...
                ViewBag._model = model;
                return View($"~/InputValues/InputValuesDisplay/{inputValue.GetType().Name.Replace("InputValue", "").Replace("`1", "")}.cshtml", inputValue);
            }
        }
    }
}
